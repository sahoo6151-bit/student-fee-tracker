...
const JWT_SECRET =
  ...
  'b3ad834a909842e9f6d755b5fa8c2a261b54fa89d0a3e6052df569aa58b2896058105747f1e014ac0c9357ac44457ad8f50020fd991c3a1b025a80c72dcd255c'
...
app.use((req, res, next) => {
  ...
  res.send = function (data) {
    logger.info(
      `RESPONSE → ${req.method} ${req.url} | Status: ${res.statusCode} | Body: ${data}`
    )
...
app.get(
  ...
  `
    ...
    const collectionRate =
      stats.total_fee_expected > 0
        ? (
            (stats.total_fee_collected / stats.total_fee_expected) *
            100
          ).toFixed(2)
        : 0
...
app.post(
  ...
  `
    ...
    const newStudent = await mysqlQuery('SELECT * FROM students WHERE id = ?', [
      result.insertId,
    ])
...
app.put(
  ...
  `
    ...
    const updatedStudent = await mysqlQuery(
...
app.delete(
  '/api/students/:id',
  asyncHandler(async (req, res) => {
    const result = await mysqlQuery('DELETE FROM students WHERE id = ?', [
      req.params.id,
    ])
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await mysqlTransaction(async (connection) => {
      ...
      if (paymentAmount > student.balance) {
        throw new Error(
          `Payment amount (${paymentAmount}) exceeds student balance (${student.balance})`
        )
      }

      // Create this once in TiDB: CREATE SEQUENCE payment_receipt_seq START WITH 1;
      const [seqResult] = await connection.query(
        'SELECT NEXTVAL(payment_receipt_seq) as next_id'
      )
      const nextId = seqResult[0].next_id

      // Format the receipt number (RCP + Year + Padded ID)
      const currentYear = new Date().getFullYear()
      const receiptNumber = `RCP${currentYear}${String(nextId).padStart(
        6,
        '0'
      )}`
    ...
    `
      ...
      // Update the student's paid_amount
      // Logic: SET paid_amount = paid_amount + NEW.amount
      await connection.query(
        `UPDATE students SET paid_amount = paid_amount + ? WHERE id = ?`,
        [paymentAmount, studentId]
      )

      // Update the student's status based on their new balance
      // This combines your CASE logic into one efficient update
      await connection.query(
        `
        UPDATE students 
        SET status = CASE 
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updatedPayment = await mysqlQuery(
      'SELECT * FROM payments WHERE id = ?',
      [req.params.id]
    )
...
app.delete(
  '/api/payments/:id',
  asyncHandler(async (req, res) => {
    const result = await mysqlQuery('DELETE FROM payments WHERE id = ?', [
      req.params.id,
    ])
...
app.get(
  '/api/fee-structures/class/:class/year/:year',
  asyncHandler(async (req, res) => {
    const query =
      'SELECT * FROM fee_structures WHERE class = ? AND academic_year = ? AND is_active = true'
...
app.get(
  '/api/fee-structures/:id/usage',
  asyncHandler(async (req, res) => {
    const feeStructureId = req.params.id

    // Get fee structure details
    const feeQuery =
      'SELECT class, academic_year FROM fee_structures WHERE id = ?'
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const checkQuery =
      'SELECT id FROM fee_structures WHERE class = ? AND academic_year = ?'
  ...
  `
    ...
    const newFeeStructure = await mysqlQuery(
...
app.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updatedFeeStructure = await mysqlQuery(
      'SELECT * FROM fee_structures WHERE id = ?',
      [req.params.id]
    )
...
app.post(
  '/api/fee-structures/bulk-status',
  asyncHandler(async (req, res) => {
...
app.get(
  '/api/fee-structures/statistics/summary',
  asyncHandler(async (req, res) => {
    const { academicYear } = req.query
